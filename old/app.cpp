...
App::~App()
{
    if (m_isLogged)
    {
        std::string onlineTopic = "dev1/marconautas/user/status/" + std::string(m_username);
        Message msg;
        msg.sender = m_username;
        msg.content = "offline";
        msg.timestamp = std::chrono::system_clock::now();
        MQTTPublish(onlineTopic.c_str(), msg, 1);
    }
    ...
    while (m_running)
    {
        ...
        if (!m_isLogged)
        {
            ...
            if (ImGui::Button("Entrar"))
            {
                ...
                // msg.content = "offline";
                // msg.timestamp = std::chrono::system_clock::now();
                // will_option.payload = (void *)&msg;
                // std::string topicName = "dev1/marconautas/user/status/" + std::string(m_username);
                // will_option.topicName = topicName.c_str();
                // will_option.qos = 1;
                ...
                MQTTSubscribe("dev1/marconautas/marcohub");
                std::string ownTopic = "dev1/marconautas/user/" + std::string(m_username);
                MQTTSubscribe(ownTopic.c_str());
                MQTTSubscribe("dev1/marconautas/user/status/+");

                std::cout << "Inscricoes realizadas com sucesso" << std::endl;
        ...
        else
        {
            if (ImGui::Begin("Usuários ativos"))
            {
                for (const auto &user : m_activeUsers)
                {
                    ...
                    {
                        std::cout << "Iniciando chat com " << user << std::endl;
                    }
                }
                ImGui::End();
            }
            ...
            {
                if (ImGui::BeginTabBar("Current Chats"))
                {
...
void App::MQTTSubscribe(const char *topic)
{
    ...
    std::cout << "Inscrito no topico " << topic << std::endl;
}

void App::MQTTPublish(const char *topic, Message& message, bool retained)
{
    int rc;

    MQTTClient_message msg = MQTTClient_message_initializer;
    msg.payload = &message;
    msg.payloadlen = sizeof(message);
    msg.qos = 1;
    ...
    std::cout << "Mensagem publicada no tópico " << topic << std::endl;
}
...
int App::onMessageArrived(void *context, char *topicName, int topicLen, MQTTClient_message *message)
{
    ...
    if(message->payloadlen == sizeof(Message)) {
        Message msg;
        memcpy(&msg, message->payload, sizeof(Message));

        std::cout << "Mensagem recebida no tópico: " << topicStr << std::endl;
        std::cout << "De: " << msg.sender << " | Conteúdo: " << msg.content << std::endl;
        std::cout << "Em: " << std::chrono::system_clock::to_time_t(msg.timestamp) << std::endl;

    } else {
        std::cout << "Tamanho da mensagem inválido: " << message->payloadlen << " esperado: " << sizeof(Message) << std::endl;
    }
